...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def max_value(gameState, agent, depth):
            if terminalTest(gameState, depth): utility(gameState)
            v = -sys.maxint
        ...
        def min_value(gameState,agent, depth):
            ...
            for a in gameState.getLegalActions(agent):
                if (agent == gameState.getNumAgents()-1):
                    ...
                    v = min(v,min_value(result(gameState,agent+1,a),angent+1,depth))
            return v 
